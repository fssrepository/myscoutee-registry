...
func settlementSourcesTx(
	...
	boundary, err := settlementBoundaryTx(ctx, tx, input.AcceptedAt)
	if err != nil {
	...
	weightSources, members, measuredWeight, err := settlementWeightSourcesTx(
		...
		input.Period,
		boundary,
		input.AcceptedAt[:len("2006-01-02")],
	)
	if err != nil {
		return store.SettlementRecord{}, err
	}
...
func settlementBoundaryTx(
	...
	transfer, err := ownershipTransferEventHeadAt(ctx, tx, acceptedAt)
	if err != nil {
		return store.LeaderboardBoundary{}, err
	}
	boundary.TransferEventIndex = transfer.EventIndex
	boundary.TransferEventHash = transfer.EventHash
	return boundary, nil
}
